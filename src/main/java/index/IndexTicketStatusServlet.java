...
/**
 * Servlet implementation class IndexTicketStatusServlet
 */
@WebServlet("/index/ticket_status")
public class IndexTicketStatusServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public IndexTicketStatusServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (session.getAttribute("userId") == null || session.getAttribute("password") == null) {
			...
			if (exist_management) {
				...
				// ギフト注文画面表示
				request.getRequestDispatcher("/WEB-INF/app/index/index_ticket_status.jsp").forward(request, response);
			} else {
