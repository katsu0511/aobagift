...
/**
 * Servlet implementation class IndexExchangeHistoryServlet
 */
@WebServlet("/index/exchange_history")
public class IndexExchangeHistoryServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public IndexExchangeHistoryServlet() {
        super();
		...
		if (session.getAttribute("userId") == null || session.getAttribute("password") == null) {
			...
			if (exist_management) {
				...
				try {
					...
					// 注文情報の注文コードを取得
					String sql1 = "SELECT POCD FROM POINF ORDER BY VLDDT ASC,POCD DESC";
				...
				// ギフト注文画面表示
				request.getRequestDispatcher("/WEB-INF/app/index/index_exchange_history.jsp").forward(request, response);
			} else {
