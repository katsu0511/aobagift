...
/**
 * Servlet implementation class IndexPurchaseServlet
 */
@WebServlet("/index/purchase")
public class IndexPurchaseServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public IndexPurchaseServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (session.getAttribute("userId") == null || session.getAttribute("password") == null) {
			...
			if (exist_management) {
				...
				// ギフト注文画面表示
				request.getRequestDispatcher("/WEB-INF/app/index/index_purchase.jsp").forward(request, response);
			} else {
